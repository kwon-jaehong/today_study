...
전체 빌드
- 매 빌드시 전체 코드를 포함해서 빌드함
- 프로젝트 전체 코드를 사용해 종속성 체크

증분 빌드
- 변경된 코드 대상만 분리해 빌드
- 프로젝트 내 모든 소스파일 및 종속성을 확인해, 변경된 코드 내용만 다시 빌드
- 변경 부분만 다시 빌드하므로 전체빌드방식보다 훨씬 빠르고 자원을 덜 사용
...
## 루트 옵션 줘야함
docker run --name jenkins -d -p 30011:8080 -v ~/jenkins:/var/jenkins_home -v /var/run/docker.sock:/var/run/docker.sock -v $(which docker):/usr/bin/docker -u root jenkins/jenkins:latest
...
# 깃랩 + 젠킨스 연동
https://www.youtube.com/watch?v=-O4tiLzYJMI
...
코드 분석은 정적 분석, 동적 분석 2가지로 나눌수 있음
