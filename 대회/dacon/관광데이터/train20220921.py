...
class CustomDataset(Dataset):
    def __init__(self,csv_path):
        ...
        ## 형태소 분리기, vocab 가방 만들 준비
        self.mecab = Mecab()
        self.TEXT = data.Field(tokenize = self.mecab.morphs, preprocessing = self.generate_bigrams)
        self.LABEL = data.LabelField(dtype = torch.float)
        
        fields = {'overview': ('text',self.TEXT), 'cat3': ('label',self.LABEL)}
        ...
        self.max_len = self.sentence_max_len(self.all_df)
    ...
    # 문장의 형태소 분리 최대 길이 구함
    def sentence_max_len(self,df):
        list_text = list(self.all_df['overview'])
        
        max_len = 0
        for i in list_text:
            if len(self.mecab.morphs(i)) > max_len:
                max_len = len(self.mecab.morphs(i))
            
        return max_len
        

dataset_temp = CustomDataset('./train.csv')
dataset_temp.__getitem__(0)
