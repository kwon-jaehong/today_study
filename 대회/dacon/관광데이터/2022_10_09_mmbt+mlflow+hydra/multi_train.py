...
def train(args, model, train_loader, optimizer, epoch, rank,criterion,scheduler):        
    ...
    for i,t_data_batch in enumerate(train_loader):
        ...
        # print(lr)
            ...
            print(f'epoch {epoch}/{args.epochs} {((i*args.batch_size)+len(train_label))*args.gpus}/{train_data_len*args.gpus} train loss {train_loss.item()/args.gpus:.4f} acc : {100*train_correct/(len(train_label)*args.gpus):.2f}% - ({train_correct}/{len(train_label)*args.gpus})')
            # print(lr)
...
def main(config: DictConfig):
    ...
    mlflow.log_dict(best_f1_score,"best_f1")
