...
for fold,epoch in fold_epoch_best_model:
    ...
    ## 폴드 모델별 웨이트 파일 불러옴
    with open(os.path.join('./mlruns',ex_id,run_id,'artifacts/val_analysis','fold_'+str(fold)+'_epoch_'+str(epoch)+'_val_label_weight.pickle'),'rb') as fr:
        weight = pickle.load(fr)
    weight_narry = np.array(list(weight.values()))
    ...
    with torch.no_grad():
        ...
        ## 웨이트를 곱해줌
        total_logit_list.append(fold_pred_softmax_val*weight_narry)
    
    
    ## 각 폴드별 예측 top1값을 csv에 저장
    result = []
...
## 폴드들의 평균 결과값에서 가장 큰값 index 가져옴
total_model_pred = np.argmax(mean_pred,1)

for j in total_model_pred:
    result.append(num2label[j])
submit["soft_weight_voting"] = result

submit.to_csv('./soft_weight_voting.csv', index=False)
