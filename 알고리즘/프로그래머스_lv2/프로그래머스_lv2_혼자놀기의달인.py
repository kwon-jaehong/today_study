...
cards = {target:index+1 for index,target in enumerate(cards)}
card_g = []
while len(cards)!=0:
    g_temp = []
    ## 첫단 뺌
    next_card_index = cards.pop(list(cards.keys())[0])
    g_temp.append(next_card_index)
    while next_card_index in list(cards.keys()):
        next_card_index = cards.pop(next_card_index,None)
        g_temp.append(next_card_index)
    card_g.append(len(g_temp))
        
card_g = sorted(card_g,reverse=True)

if len(card_g)==0:
    answer = 0
else:
    answer = card_g[0] * card_g[1]
